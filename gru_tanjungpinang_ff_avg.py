...
# Mengimpor data
link_data_set = 'https://raw.githubusercontent.com/ReziAfrialdi/dataset-skripsi/main/laporan_iklim_harian_tanjungpinang_ff_avg.csv'
data_kecepatan_angin = pd.read_csv(link_data_set)
data_ff_avg_tgpinang = pd.read_csv(link_data_set)
...
# Load model
model_filename='GRU_FF_AVG_TGPINANG.keras'
model = tf.keras.models.load_model(model_filename)

def predict_ff_avg_tgpinang(input_data):
...
def predict_original_data_ff_avg_tgpinang(input_data) :
...
def predict_multiple_day_ff_avg_tgpinang(day):
    data_y= Y_test
    ...
    for i in range(day):
        ...
        prediction = predict_ff_avg_tgpinang(input_x)
